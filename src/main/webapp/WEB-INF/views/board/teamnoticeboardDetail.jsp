...
<style>
	...
	#listButton, #writeButton, #deleteButton, #updateButton, #reportButton, #registerBtn1, #registerBtn2{
		font-size: 15px;
		height: 30px;
    	margin : 5px;
...
<body>
	...
	<div id="content">
		<table>
			...
			<tr>
				<th colspan="4">
					<input type = "button" class="btn btn-outline-dark" id="listButton" onclick="location.href='./teamnoticeboardList.do?teamIdx=${teamIdx}'" value="리스트"/>
					&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
					<c:if test="${dto.userId eq loginId }">
					<input type = "button" class="btn btn-outline-dark" id="registerBtn1" onclick="location.href='./teamnoticeboardUpdate.go?bidx=${dto.boardIdx}'" value="수정"/>
					&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
					<input type = "button" class="btn btn-outline-dark" id="registerBtn2" value="삭제" onclick="if(confirm('정말로 삭제하시겠습니까?')){location.href='./teamnoticeboardDelete.do?bidx=${dto.boardIdx}';}">			
					</c:if>
				</th>
			</tr>
		</table>
