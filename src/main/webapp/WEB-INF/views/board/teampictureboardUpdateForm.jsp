...
<body>
	...
	<div id="content">
	<form action="teampictureboardUpdate.do" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
		...
		<table>
			<tr>
				<th>제목</th>
					<td>
						<input type="text" name="subject" value = "${dto.get(0).subject}" id="subjectInput" maxlength="60" style="width : 950px ; height : 30px" 
			...
			<tr>
				<th>작성자</th>
				<td><input type="text" name="userId" value = "${dto.get(0).userId}"style="border:none; background-color: #f8f9fa ; text-align:center;"readonly/></td>
			</tr>
			<tr>
				<th>내용</th>
				<td><textarea name="content" id="contentInput" style="width : 950px; height : 200px; resize: none">${dto.get(0).content}</textarea></td>
			</tr>
			<tr>
				<th>사진</th>
				<td>
					...
					<c:if test="${dto.get(0).photoName ne null}">
						<img src = "/photo/${dto.get(0).photoName}" style="width: 50%; height: 50%;"/>
					</c:if>
				</td>
			</tr>
			<tr>
				<th colspan="2">
					<button class="btn btn-outline-dark" id="writeButton">저장</button>
				</th>
			</tr>	
		</table>
	</form>
